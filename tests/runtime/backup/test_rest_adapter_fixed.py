...
async def test_predict_endpoint_success():
    ...
    # Since our mock setup returns the mock itself, just verify the endpoint was called successfully
...
def test_main_block():
    ...
    # We need to import the module without executing the main block first
    with patch.object(sys.modules['__main__'], '__file__', '/workspaces/copilot/runtime/rest_adapter.py'):
        import runtime.rest_adapter
    
        # Now execute the main block directly
        from importlib.util import spec_from_file_location, module_from_spec
        spec = spec_from_file_location("__main__", "/workspaces/copilot/runtime/rest_adapter.py")
        main_module = module_from_spec(spec)
        
        # Set the __name__ to __main__ to trigger the main block
        main_module.__name__ = "__main__"
        
        # Add the necessary modules to main_module's globals
        main_module.__dict__['uvicorn'] = sys.modules['uvicorn']
        main_module.__dict__['app'] = runtime.rest_adapter.app
